...
export default function Dashboardholiday() {
  ...
  const fetchData = async (query: string = "", page: number = 1) => {
    try {
      ...
      const response = await axios.get(
        `/api/vendors${query ? `?search=${query}&` : "?"}page=${page}`,
        {
          headers: {
  ...
  useEffect(() => {
    setConfig({
      ...
      tableColumns: {
        title: "Vendor",
        description: "Manage Vendor and view their details.",
        headers: [
          { label: "Vendor Name", key: "one" },
          { label: "Organization Name", key: "two" },
          { label: "GST Number", key: "three" },
          { label: "Website", key: "four" },
  ...
  // Add pagination handlers before the return statement
  ...
  const mappedTableData = data?.map((item) => {
    ...
    return {
      ...
      two: item?.organization_name || "NA",
      three: item?.gst_number || "NA",
      four: item?.website || "NA",

      delete: "/vendors/" + item?.id,
    };
  });
