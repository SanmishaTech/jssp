...
export default function Dashboardholiday() {
  ...
  const fetchData = async (query: string = "", page: number = 1) => {
    try {
      ...
      const response = await axios.get<ApiResponse>(
        `/api/assetcategories${query ? `?search=${query}&` : "?"}page=${page}`,
        {
          headers: {
      ...
      setData(response.data.data.AssetCategories);
  ...
  useEffect(() => {
    setConfig({
      ...
      tableColumns: {
        ...
        description: "Manage Asset Category and view their details.",
        headers: [
          { label: "Asset Category", key: "one" },
  ...
  const mappedTableData = data.map((item) => ({
    id: item.id,
    one: item.category_name || "Unknown",
    delete: "/asset_categories/" + item.id,
  }));
