...
export default function Dashboardholiday() {
  ...
  const fetchData = async (query: string = "", page: number = 1) => {
    try {
      ...
      const response = await axios.get<ApiResponse>(
        `/api/academic_years${query ? `?search=${query}&` : "?"}page=${page}`,
        {
          headers: {
      ...
      setData(response.data.data.AcademicYears);
  ...
  useEffect(() => {
    setConfig({
      ...
      tableColumns: {
        title: "Academic Year",
        description: "Manage Academic Year and view their details.",
        headers: [
          { label: "Academic Year", key: "one" },
  ...
  const mappedTableData = data.map((item) => ({
    id: item.id,
    one: item.academic_year || "Unknown",
    delete: "/academic_years/" + item.id,
  }));
