...
interface EducationCertificateUploadProps {
...
export default function EducationCertificateUpload({ 
  form, 
  existingDocuments = [], 
  onAddDocuments, 
  onRemoveDocument,
  onRemoveNewDocument
}: EducationCertificateUploadProps) {
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>Upload education certificates (PDF, DOC, XLS, Images, etc.)</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="text-sm text-gray-500 flex items-center justify-between">
            <span>{existingDocuments.length + selectedDocuments.length} of 10 certificates</span>
            {existingDocuments.length + selectedDocuments.length > 0 && (
              <Button 
                ...
                onClick={() => {
                  ...
                  if (window.confirm("Are you sure you want to remove all certificates?")) {
                    ...
                    toast.success("All certificates removed. Changes will be saved when you submit the form.");
                  }
                }}
                className="text-xs"
              >
