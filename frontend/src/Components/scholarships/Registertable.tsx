...
interface DashboardProps {
  ...
  onProductAction: (action: string, product: Scholarship) => Promise<void>;
...
export default function Dashboardholiday() {
  ...
  const fetchData = async (query: string = "", page: number = 1) => {
    try {
      ...
      const response = await axios.get<ApiResponse>(
        `/api/scholarships${query ? `?search=${query}&` : "?"}page=${page}`,
        {
          headers: {
      ...
      setData(response.data.data.Scholarship);
  ...
  const handleProductAction = async (action: string, product: Scholarship) => {
    if (action === "edit") {
      setSelectedScholarshipId(product.id);
      setIsEditDialogOpen(true);
    } else if (action === "delete") {
      try {
        const response = await axios.delete(`/api/scholarships/${product.id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
