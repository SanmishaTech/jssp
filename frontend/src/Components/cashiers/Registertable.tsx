...
interface DashboardProps {
  ...
  onProductAction: (action: string, product: Cashier) => Promise<void>;
...
export default function Dashboardholiday() {
  ...
  const fetchData = async (query: string = "", page: number = 1) => {
    try {
      ...
      const response = await axios.get<ApiResponse>(
        `/api/cashiers${query ? `?search=${query}&` : "?"}page=${page}`,
        {
          headers: {
  ...
  const handleProductAction = async (action: string, product: Cashier) => {
    if (action === "edit") {
      setSelectedCashierId(product.id);
      setIsEditDialogOpen(true);
    } else if (action === "delete") {
      try {
        const response = await axios.delete(`/api/cashiers/${product.id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
