...
interface DashboardProps {
  ...
  onProductAction: (action: string, product: Room) => Promise<void>;
...
export default function Dashboardholiday() {
  ...
  const fetchData = async (query: string = "", page: number = 1) => {
    try {
      ...
      const response = await axios.get<ApiResponse>(
        `/api/requisitions${query ? `?search=${query}&` : "?"}page=${page}`,
        {
          headers: {
      ...
      setData(response.data.data.Requisition);
  ...
  useEffect(() => {
    setConfig({
      ...
      tableColumns: {
        title: "Requisition",
        description: "Manage Requisition and view their details.",
        headers: [
          { label: "Requisition Number", key: "one" },
          { label: "Requisition Name", key: "two" },
  ...
  const handleProductAction = async (action: string, product: Room) => {
    if (action === "edit") {
      setSelectedRoomId(product.id);
      setIsEditDialogOpen(true);
    } else if (action === "delete") {
      try {
        const response = await axios.delete(`/api/requisitions/${product.id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const mappedTableData = data.map((item) => ({
    id: item.id,
    one: item.asset_name || "Unknown",
    two: item.description || "NA",
     delete: "/requisitions/" + item.id,
  }));
